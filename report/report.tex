...
In particular, networks were evolved with real-time NEAT, which does not use generations - it instead replaces agents' networks throughout the simulation. Despite this, however, the search space is eventually depleted of crumbs and must be reset. Simulations are therefore divided into \textit{episodes} which last a fixed interval, and  each episode begins by distributing agents and crumbs around the environment. Agents' networks are continually swapped out throughout an episode according to the rtNEAT algorithm.
